  ...
  return (
    <SpaceBetween size="xxs" alignItems="center" direction="vertical">
      <div style={{
        ...
        height: 40,
        borderRadius: '50%',
        background: 'rgba(0, 108, 224, 0.15)',
        display: 'flex',
        alignItems: 'center',
...
export default function AuthPage() {
  ...
  return (
    <div style={{
      ...
      justifyContent: 'center',
      minHeight: '100vh',
      background: `linear-gradient(135deg, ${awsui.colorBackgroundHomeHeader} 0%, #0a1628 100%)`,
    }}>
        ...
        <SpaceBetween size="l">
          <Box textAlign="center" padding={{ bottom: 's' }}>
            <SpaceBetween size="xxs" alignItems="center" direction="vertical">
              ...
              <Box variant="h1" color="text-status-info" fontSize="display-l" fontWeight="bold">
                Stakflo
              </Box>
              <Box variant="p" color="text-status-inactive" fontSize="body-m">
                AI-Powered Continuous Compliance. Always Audit-Ready.
              </Box>
            </SpaceBetween>
          </Box>
          ...
          <Container header={<Header variant="h2">{isSignUp ? 'Create account' : 'Sign in'}</Header>}>
            <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
              <Form
                actions={
                  <Button variant="primary" loading={loading} fullWidth onClick={handleSubmit}>
              ...
              >
                <SpaceBetween size="m">
                  ...
                  <FormField key="email" label="Email">
                    <Input type="email" value={email} onChange={({ detail }) => setEmail(detail.value)} placeholder="you@company.com" />
                  </FormField>
                  <FormField key="password" label="Password">
                    <Input type="password" value={password} onChange={({ detail }) => setPassword(detail.value)} placeholder="Enter password" />
                  </FormField>
                  <Box key="link" textAlign="center">
                    <Link onFollow={() => { setIsSignUp(!isSignUp); setError(''); setSignUpSuccess(false); }}>
